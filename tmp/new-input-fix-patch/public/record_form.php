...
$popupReturnTarget = in_array($_GET['return'] ?? '', ['dashboard', 'records'], true)
    ? (string) $_GET['return']
    : '';
$isPopup = $id > 0 && ($_GET['popup'] ?? '') === '1' && $popupReturnTarget !== '';
$popupDefaultCloseUrl = $popupReturnTarget === 'dashboard'
    ? '/dashboard.php?city_tab=review'
    : '/records.php';
$popupAllowedClosePath = $popupReturnTarget === 'dashboard'
    ? '/dashboard.php'
    : '/records.php';
...
$canChooseReferralCommittee = (
    ...
    && (
        (can_city_secretary_action() && (($record['status'] ?? '') === 'Received' || ($record['document_type'] ?? '') === 'Committee Referrals'))
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $controlNumber = strtoupper(trim($_POST['control_number'] ?? ''));
    ...
    $data = [
        ...
        'status' => $documentType === 'Certified Urgent' && (!$id || ($record['status'] ?? '') === 'Received')
            ? 'For Plenary Session'
    ...
    if (
        ...
        try {
            ...
            $movement = $pdo->prepare('INSERT INTO record_movements (record_id, from_status, to_status, from_location, to_location, notes, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?)');
            $movement->execute([
                (int) $targetRecord['id'],
                $targetRecord['status'],
                $targetRecord['status'],
    ...
    if ($id) {
        ...
        try {
            $saveWithUniqueControlNumber(static function () use (&$data, $id): void {
                $stmt = db()->prepare('UPDATE records SET control_number=?, title=?, document_type=?, origin=?, client_name=?, contact_number=?, client_email=?, committee_id=?, assigned_user_id=?, receiving_clerk_id=?, priority=?, status=?, received_date=?, due_date=?, current_location=?, remarks=?, updated_by=? WHERE id=?');
                $stmt->execute([
                    ...
                    $data['current_location'], $data['remarks'], $data['updated_by'], $id,
                ]);
            });
        } catch (Throwable $error) {
        ...
        if ($oldStatus !== $data['status'] || $oldLocation !== $data['current_location'] || $isReceivingStaffCommentResponse || $hasCityReceivingStaffComment) {
            $move = db()->prepare('INSERT INTO record_movements (record_id, from_status, to_status, from_location, to_location, notes, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?)');
            if ($isReceivingStaffCommentResponse) {
                ...
                $movementNotes = $data['remarks'];
            }
            $move->execute([$id, $oldStatus, $data['status'], $oldLocation, $data['current_location'], $movementNotes, current_user()['id']]);
        }
        ...
        $move = db()->prepare('INSERT INTO record_movements (record_id, to_status, to_location, notes, updated_by) VALUES (?, ?, ?, ?, ?)');
        $creationNote = $data['document_type'] === 'Certified Urgent'
            ? 'Certified Urgent record created and routed directly for plenary session.'
            : 'Record created.';
        $move->execute([$id, $data['status'], $data['current_location'], $creationNote, current_user()['id']]);
...
<div class="page-head">
    <div>
        <h1 id="record_form_title"><?= $id && can_city_secretary_action() && ($record['status'] ?? '') === 'Received' ? 'Review Record' : ($id && is_administrative_document_type($record['document_type'] ?? '') ? 'Forward to' : ($id ? 'Edit Record' : 'New Record')) ?></h1>
...
<form method="post" class="<?= $isPopup ? 'form-grid record-form-panel' : 'panel form-grid record-form-panel' ?>" enctype="multipart/form-data">
    ...
    <label>Record Type
        <?php if ($canChooseDocumentType): ?>
            <select name="document_type" id="document_type">
                <?php foreach ($documentTypes as $typeOption): ?>
    ...
    <label class="full standard-title-field">Title
        <textarea name="title" required rows="4" maxlength="1000"><?= e($record['title']) ?></textarea>
    </label>
        ...
        <section class="full review-choice-panel record-tagging-field">
            ...
            <datalist id="merge_record_candidates">
                <?php foreach ($mergeCandidateRecords as $candidate): ?>
                    <option value="<?= e($candidate['control_number']) ?>"><?= e($candidate['document_type'] . ' - ' . $candidate['title']) ?></option>
                <?php endforeach; ?>
            </datalist>
        </section>
    <?php endif; ?>
    ...
    <?php if ($canChooseReferralCommittee): ?>
        <label class="legislative-field full">Committee(s)
            ...
            <span class="muted"><?= can_city_secretary_action() ? 'Check one or more committees. A correction note keeps the review pending; approval without a correction note finalizes it for committee action and printing.' : 'Check one or more committees. This is not final until reviewed by the SP Secretary.' ?></span>
        </label>
...
const canFinalizeCommitteeAssignment = <?= can_city_secretary_action() ? 'true' : 'false' ?>;
...
const toggleRecordFields = () => {
    ...
    if (recordTypeHelp) {
        recordTypeHelp.textContent = isCertifiedUrgent
...
if (documentTypeSelect) {
    documentTypeSelect.addEventListener('change', () => {
        ...
        if (controlInput) {
            if (documentTypeSelect.value !== originalDocumentType && nextControlNumbers[documentTypeSelect.value]) {
